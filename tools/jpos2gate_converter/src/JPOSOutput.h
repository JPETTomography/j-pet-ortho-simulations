...
// Thu Feb 15 11:11:36 2018 by ROOT version 6.08/06
// from TTree tree/Tree with events and histograms
// found on file: data/jpos.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   const Int_t kMaxfPrimaryPhoton = 1;
 ...
 //Event           *event_split;
   ...
   Int_t           fDecayType_;
   Bool_t          fPassFlag_;
   std::vector<bool>    fPrimaryPhoton_;
   ...
   TBranch        *b_event_split_fPrimaryPhoton_;   //!
...
#ifdef JPOSOutput_cxx
JPOSOutput::JPOSOutput(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("data/jpos.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("data/jpos.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("data/jpos.root:/0_0_0_0_0_0");
...
JPOSOutput::~JPOSOutput()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t JPOSOutput::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t JPOSOutput::LoadTree(Long64_t entry)
{
...
void JPOSOutput::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("fPrimaryPhoton_", &fPrimaryPhoton_, &b_event_split_fPrimaryPhoton_);
...
void JPOSOutput::Show(Long64_t entry)
{
...
Int_t JPOSOutput::Cut(Long64_t entry)
{
