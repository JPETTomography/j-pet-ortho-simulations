...
// from TTree Hits/The root tree for hits
// found on file: data/gate.root
//////////////////////////////////////////////////////////
...
#ifdef GateOutput_cxx
GateOutput::GateOutput(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("data/gate.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("data/gate.root");
      }
...
Int_t GateOutput::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t GateOutput::LoadTree(Long64_t entry)
{
...
void GateOutput::Init(TTree *tree)
{
...
void GateOutput::Show(Long64_t entry)
{
...
Int_t GateOutput::Cut(Long64_t entry)
{
