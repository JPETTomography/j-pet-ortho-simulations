...
void InitialCuts::AddCuts(Event& event)
{
    ...
    for(int ii=0; ii<3; ii++)
    {
        ...
        if(event.GetEmissionPointAt(ii)!=nullptr && event.GetFourMomentumAt(ii)!=nullptr)
        {
            fNumberOfGammas_++;
            bool geo_pass = GeometricalAcceptance_(event.GetEmissionPointAt(ii), event.GetFourMomentumAt(ii));
            bool inter_pass = DetectionCut_();
            ...
            {
                fAcceptedGammas_++;
                fH_en_pass_->Fill(event.GetFourMomentumAt(ii)->Energy());
                fH_p_pass_->Fill(event.GetFourMomentumAt(ii)->P());
                fH_phi_pass_->Fill(event.GetFourMomentumAt(ii)->Phi());
                fH_cosTheta_pass_->Fill(event.GetFourMomentumAt(ii)->CosTheta());
            }
            else
            {
                fH_en_fail_->Fill(event.GetFourMomentumAt(ii)->Energy());
                fH_p_fail_->Fill(event.GetFourMomentumAt(ii)->P());
                fH_phi_fail_->Fill(event.GetFourMomentumAt(ii)->Phi());
                fH_cosTheta_fail_->Fill(event.GetFourMomentumAt(ii)->CosTheta());
            }
        }
    }
    if(geo_event_pass)
...
void InitialCuts::FillValidEventHistograms_(const Event& event)
{
    ...
    for(int ii=1; ii<3; ii++)
        ...
        {
            fH_en_pass_event_->Fill(event.GetFourMomentumAt(ii)->Energy());
            minIndex = event.GetFourMomentumAt(ii)->E() < event.GetFourMomentumAt(minIndex)->E() ? ii : minIndex;
            maxIndex = event.GetFourMomentumAt(ii)->E() > event.GetFourMomentumAt(maxIndex)->E() ? ii : maxIndex;
        }
    fH_en_pass_low_->Fill(event.GetFourMomentumAt(minIndex)->Energy());
    fH_en_pass_high_->Fill(event.GetFourMomentumAt(minIndex)->Energy());
    if(fDecayType_==THREE || fDecayType_==TWOandONE)
    {
         if(fDecayType_==THREE)
         {
             ...
             fH_en_pass_mid_->Fill(event.GetFourMomentumAt(midIndex)->Energy());
         }
        fH_12_23_pass_->Fill(event.GetFourMomentumAt(0)->Angle(event.GetFourMomentumAt(1)->Vect()), \
                             event.GetFourMomentumAt(1)->Angle(event.GetFourMomentumAt(2)->Vect()), event.GetWeight());
        fH_12_31_pass_->Fill(event.GetFourMomentumAt(0)->Angle(event.GetFourMomentumAt(1)->Vect()), \
                             event.GetFourMomentumAt(2)->Angle(event.GetFourMomentumAt(0)->Vect()), event.GetWeight());
        fH_23_31_pass_->Fill(event.GetFourMomentumAt(1)->Angle(event.GetFourMomentumAt(2)->Vect()), \
                             event.GetFourMomentumAt(2)->Angle(event.GetFourMomentumAt(0)->Vect()), event.GetWeight());
    }
    else if(fDecayType_ == TWO)
        fH_12_pass_->Fill(event.GetFourMomentumAt(0)->Angle((event.GetFourMomentumAt(1))->Vect()), event.GetWeight());
    else
    {
...
void InitialCuts::FillInvalidEventHistograms_(const Event& event)
{
    if(fDecayType_==THREE || fDecayType_==TWOandONE)
    {
        fH_12_23_fail_->Fill(event.GetFourMomentumAt(0)->Angle(event.GetFourMomentumAt(1)->Vect()), \
                             event.GetFourMomentumAt(1)->Angle(event.GetFourMomentumAt(2)->Vect()), event.GetWeight());
        fH_12_31_fail_->Fill(event.GetFourMomentumAt(0)->Angle(event.GetFourMomentumAt(1)->Vect()), \
                             event.GetFourMomentumAt(2)->Angle(event.GetFourMomentumAt(0)->Vect()), event.GetWeight());
        fH_23_31_fail_->Fill(event.GetFourMomentumAt(1)->Angle(event.GetFourMomentumAt(2)->Vect()), \
                             event.GetFourMomentumAt(2)->Angle(event.GetFourMomentumAt(0)->Vect()), event.GetWeight());
    }
    else if(fDecayType_ == TWO)
        fH_12_fail_->Fill(event.GetFourMomentumAt(0)->Angle((event.GetFourMomentumAt(1))->Vect()), event.GetWeight());
    else
    {
         throw("Invalid no of decay products in InitialCuts!");
    }
